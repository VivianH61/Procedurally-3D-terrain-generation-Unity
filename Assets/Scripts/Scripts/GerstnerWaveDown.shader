...
Shader "Unlit/GerstnerWaveDown"
{
	...
	SubShader
	{
		...
		//	/*float3 objNormal = CalculateWavesNormal(i.worldPos, _QA, _A, _S, _Dx, _Dz, _L);
		//	objNormal = mul((float3x3)unity_WorldToObject, normalize(objNormal));
		//	objNormal = normalize(objNormal+pixelNormal);*/
		//
		//	float3 objCameraPos = mul(unity_WorldToObject, _WorldSpaceCameraPos.xyz);
		//	float3 viewVector = i.objPos - objCameraPos;
		//	float3 nNormal, fNormal;
		//	float2 d = BoxIntersection(objCameraPos.xyz, (viewVector), _BoxSize, nNormal, fNormal);
		//	float3 near = objCameraPos.xyz + normalize(viewVector) * d.x;
		//	float3 far = objCameraPos.xyz + normalize(viewVector) * d.y;
		...
		//	float fresnel = FastFresnel(normalize(-viewVector), objNormal, _FresnelFactor);

		//	float4 reflectColor = tex2D(_ReflectionTex, projUV + objNormal.xz*_ReflectOffset);
		//	col.xyz = lerp(col.xyz, reflectColor.xyz, fresnel);
		//	//col.xyz = reflectColor.xyz;
...
#include "NPRWaterTool.cginc"
			...
			float4 frag(v2f i) : SV_Target
			{
				...
				float3 outScattering;
				float3 inScattering;
				ComputeScattering((len + len2) * _HeightAtten.z, _DiffuseRadiance, _Attenuation, _Kd, outScattering, inScattering);
				reflectColor.xyz = reflectColor * outScattering + inScattering;

				ComputeScattering(len2 * _HeightAtten.z, _DiffuseRadiance, _Attenuation, _Kd, outScattering, inScattering);

				float3 refrV = refract((viewVector), objNormal, 1/1.33f);
				float4 refractColor = texCUBE(_Skybox, refrV);
				refractColor.xyz = refractColor.xyz * outScattering + inScattering;
