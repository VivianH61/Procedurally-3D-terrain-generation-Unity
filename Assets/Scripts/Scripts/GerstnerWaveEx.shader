...
Shader "Unlit/GerstnerWave"
{
	...
	SubShader
	{
		...
		float4 frag(v2f i) : SV_Target
		{
			...
			float3 refractIn = refract(normalize(viewVector), objNormal, 1.33f);
			float3 inPos = i.objPos - refractIn;
			float3 nNormal, fNormal;
			float2 d1 = BoxIntersection(inPos, refractIn, _BoxSize, nNormal, fNormal);
			float near1 = inPos + normalize(refractIn)*d.x;
			float far1 = inPos + normalize(refractIn)*d.y;
			float len1 = length(far - near);
